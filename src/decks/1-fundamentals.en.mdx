...
- Back in the day, these tools had to be picked manually:
...
- **.eslintrc.cjs** : eslint configuration (provides js validation)
...
With JavaScript language, there is a linter rule asking to provide prop-types. This rule isn't useful. [PropTypes aren’t commonly used in modern React.](https://legacy.reactjs.org/docs/typechecking-with-proptypes.html)

To eliminate this useless warning, we add this rule in the file `eslintrc.cjs`.
...
You'll need to install the dependencies (in the `node_modules` folder)
...
- Modify line 19 of `src/app.jsx` and save,
  - This will automatically update the browser content, no need to _refresh_
- This feature is called _React Fast Refresh_ (also _Hot Module Reload_)
...
- Declarative way to generate HTML
...
- JSX s first bundled with React
- Adopted by other other technologies:
  - **Preact** (Lght and compatible)
  - **Vue** (Versatile)
  - **Inferno** (Fast)
  - **Svelte** (Optimized)
  - **Solid** (Composable)
- JSX is not part of the JavaScript/EcmaScript standard (not Today)

---

### JSX, behind the curtain (1/3)
...
Type `o`to browse to: http://localhost:4173
