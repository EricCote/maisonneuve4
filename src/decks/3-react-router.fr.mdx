...
Avec du JavaScript, on peut changer le rendu de react. On affiche le composant en fonction de la page demandée.
...
export default function Page1() {
  const [movies, refreshFilms] = useMarvelMovies();

  return (
    <div>
      <h1>Page 1: Marvel Movies </h1>
      <ul>
        {movies.map((movie) => {
          return (
            <li key={movie.id}>
...
export default function Page3() {
  ...
  const txtAddEmail = useRef();
  if (txtAddEmail === null) console.log('noooo!');

  return (
    <>
      <h1>Page 3: Email Form</h1>
      <form
        onSubmit={(evt) => {
          evt.preventDefault();
      ...
      >
        ...
        {isAdding ? (
          <div className='my-3'>
            <label>
              <input className='' ref={txtAddEmail} placeholder='Add Email' />
              <button
                onClick={() => {
                  setContact({
                    ...contact,
...
Il faudrait donc remplacer les liens normaux par des liens qui ne téléchargent pas.
...
function changeAllLinksToHashes() {
  ...
  for (const link of document.querySelectorAll(':link')) {
    ...
    if (url.hostname === window.location.hostname) {
      ...
      //Workaround for strictMode: Correctly add the hash when called a second time.
...
export default function Page2() {
  ...
  return (
    <div>
      <h1>Page 2: Author list with filter</h1>
      <input
        value={filter}
        onChange={(evt) => {
          setFilter(evt.currentTarget.value);
...
export default function App() {
  ...
  //Note 1: doesn't show the proper url on this code sandbox.
  //       (shows url on real browsers)
  //Note 2: Is still not implemented by Safari and Firefox.
  ...
  useEffect(() => {
    ...
    window.navigation.addEventListener('navigate', (evt) => {
      if (!evt.canIntercept) return; //is it a local page that we can intercept?
      evt.intercept({
        //in that case, intercept the navigation (cancels browser navigation)
        handler: () => {
