...
On recommande de d'englober chaque block case entre des accolades `{` et `}`  pour que les variables des blocs soient isolées les unes des autres. De plus, chaque `case` se termine par `return` (ou `break`). Si on oublie le `return`, le code continue au prochain case, ce qui est une erreur.
...
## Défi : Clear the input on sending a message {/* clear-the-input-on-sending-a-message */}
...
## Défi: Restore input values when switching between tabs {/* restore-input-values-when-switching-between-tabs */}
