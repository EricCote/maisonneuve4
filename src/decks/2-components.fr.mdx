...
**Règle** Quand on isole le composant dans son fichier, il faut utiliser `export default`
...
function Recipe({ nbPersonnes }) {
  return (
    <ol>
      ...
      <li>Ajouter {nbPersonnes} cuillères à soupe de thé,
...
function Cup() {
  ...
  invité = invité + 1;
  return <h2>Tasse pour l'invité #{invité}</h2>;
}
