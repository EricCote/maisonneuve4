...
- `Index.jsx` doit importer `react-dom/client`

- `ReactDOM.createRoot` permet de créer une racine React pour le `<div>`.
...
La décomposition (destructuring) avec `{}` est populaire.
  ...
  return (
    <section>
      ...
      <ul>
        ...
        <Item estRangé={false} name='Guitare' />
      </ul>
    </section>
  );
}
...
Avec une structure plus complexe, fltrons et affichons:
...
`.filter` est une fonction standard.
...
export function getImageUrl(personne) {
  return (
    'https://upload.wikimedia.org/wikipedia/commons/thumb/' + personne.imageId
  );
}
```
...
<aside style={{display:'grid', gridTemplateColumns:"2fr 1fr", columnGap:5, fontSize:"0.8em"}}>
...
`StrictMode` sera ignoré en production.
...
Le code précédent est correct, car `tasses` est une variable locale. On peut modifier les variables locales sans problèmes.

Si `tasses` était une variable globale, ce serait mauvais.
  ...
  return (
    <ul>
      {témoignages.map((témoignage) => (
        <li key={témoignage.id}>{témoignage.label}</li>
      ))}
    </ul>
  );
}
